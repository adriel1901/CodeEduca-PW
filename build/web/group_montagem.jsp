...
<body>
    <div class="container">
        ...
        <% 
            // Exibe mensagens de retorno (sucesso na criação, etc.)
        ...
        <% if (listGroup == null || listGroup.isEmpty()) { %>
            <p>Você ainda não criou nenhum grupo de questões.</p>
        <% } else { %>
                ...
                <tbody>
                    <% for (QuestionGroup group : listGroup) { %>
                        <tr>
                            ...
                            <td><%= new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm").format(group.getCreatedAt()) %></td>
                            <td class="actions">
                                <%-- Ações futuras: Ver, Editar, Excluir --%>
                                <a href="QuestionServlet?groupId=<%= group.getGroupId() %>">Ver Questões</a>
                                <a href="GroupServlet?action=edit&id=<%= group.getGroupId() %>">Editar</a>
                                <a href="GroupServlet?action=delete&id=<%= group.getGroupId() %>" onclick="return confirm('Tem certeza que deseja excluir este grupo?');">Excluir</a>
                            </td>
                        </tr>
                    <% } %>
                </tbody>
